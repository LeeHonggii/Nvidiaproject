...
def run_video_editor(parameter):
    ...
    if "vector" not in parameter["meta_info"] or parameter["meta_info"]["vector"] == "points":
        ...
        for i in range(len(cross_list)):
            ...
            if "rotation" not in parameter["meta_info"] or parameter["meta_info"]["rotation"] == "YES":
            ...
            if x12 - x1 < 0:
                print(f"!!!ERROR wrong coordination x12 is smaller than x1. x1({x1}), y1({y1}), x12-x1({x12-x1}), y12-y1({y12-y1})")
                return ()
