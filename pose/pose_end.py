...
def create_transition_video(max_transformation_order, csv_to_video_mapping, output_file="output.mp4"):
    # 비디오 캡쳐 객체 생성
    video_caps = {video_file: cv2.VideoCapture(video_file) for video_file in csv_to_video_mapping.values()}

    # 첫 번째 비디오의 프레임 정보 추출
    first_video_file = csv_to_video_mapping[max_transformation_order[0][1]]
    width = int(video_caps[first_video_file].get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(video_caps[first_video_file].get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = video_caps[first_video_file].get(cv2.CAP_PROP_FPS)
    ...
    # 최대 변환 순서대로 비디오 작성
    for i, (start_frame, csv_file) in enumerate(max_transformation_order):
        video_file = csv_to_video_mapping[csv_file]

        if i < len(max_transformation_order) - 1:
            end_frame = max_transformation_order[i + 1][0] - 1
        else:
            # 마지막 변환 구간 이후의 프레임 설정
            end_frame = int(video_caps[video_file].get(cv2.CAP_PROP_FRAME_COUNT)) - 1

        if current_video_file != video_file:
            current_video_file = video_file
            video_caps[current_video_file].set(cv2.CAP_PROP_POS_FRAMES, start_frame)

        while current_frame_index <= end_frame:
            ret, frame = video_caps[current_video_file].read()
            if not ret:
                break
            out.write(frame)
            current_frame_index += 1

    # 모든 비디오 캡쳐 객체와 비디오 라이터 객체 해제
    for cap in video_caps.values():
        cap.release()
    out.release()

# 최대 변환 순서를 기반으로 비디오 생성
create_transition_video(max_transformation_order, VIDEO_FILES)
print("비디오 생성 완료")
